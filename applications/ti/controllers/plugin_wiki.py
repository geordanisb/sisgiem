...
def index():
    ...
    elif auth.has_membership("editor_ti"):
        id = auth.id_group("administrador")
        pages = db(w.role!=id).select(orderby=w.slug)#1 es el role del administrador
    else:
        pages = db(w.role==None)(w.is_active==True)(w.is_public==True).select(orderby=w.slug)

    if (auth.has_membership("editor_ti") or auth.has_membership("administrador")):
...
def page():
    ...
    #elif not (auth.has_membership("editor_ti") or auth.has_membership("administrador")) and (not page or not page.is_public or not page.is_active):
    #    raise HTTP(404)
    elif page and page.role and not auth.has_membership(page.role):    
        raise HTTP(404,"No tiene los privilegios suficientes {}".format(A("Regresar",_href=URL("plugin_wiki","index"))))
...
def page_archive():
    ...
    page = h(id)
    if not page or (not auth.has_membership("editor_ti") and (not page.is_public or not page.is_active)):
        raise HTTP(404)
    elif page and page.role and not auth.has_membership(page.role):
        raise HTTP(404)
...
#@auth.requires(plugin_wiki_editor)
@auth.requires(auth.has_membership("editor_ti") or auth.has_membership("administrador"))
def page_edit():
    """
    edit a page
    """
    ...
    if (page.role == auth.id_group("administrador")) and not auth.has_membership("administrador"):
        raise HTTP(404)
       
    form = SQLFORM(w,page,next=URL(r=request,f='page',args=request.args),deletable=True)
    if form.process().accepted:redirect(URL("plugin_wiki","index"))
