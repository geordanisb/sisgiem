...
DEFAULT = {
    'editor' : auth.user and auth.has_membership(role='editor'), # if current user a editor?
...
for _f in ['plugin_wiki/ui/css/%s/jquery-ui-1.8.5.custom.css' % plugin_wiki_theme,
           ...
           'plugin_wiki/rating/jquery.rating.js']:pass
...
""" % dict(callback=callback,colnames=json(colnames),
    ...
    def mediaplayer(src,width=400,height=250):
        ...
        return XML('<video controls><source src="{}" width="{}" height="{}">Su navegador no soporta este tipo de video!</video>'.format(src,width,height))
